...
describe("MyNFT", function () {
  ...
  it("Should mint a new NFT to the recipient", async function () {
    await nftContract.connect(ownerWallet).mint(recipientWallet.address, 1);
    const balance: BigNumber = await nftContract.balanceOf(recipientWallet.address);
  ...
  it("Should have correct token URI after minting", async function () {
    ...
    expect(tokenURI).to.equal("https://mybaseuri.com/token/" + tokenId);
  });

  it("Should allow owner to mint multiple NFTs", async function () {
    await nftContract.connect(ownerWallet).mint(recipientWallet.address, 2);
    await nftContract.connect(ownerWallet).mint(recipientWallet.address, 3);
    const balance: BigNumber = await nftContract.balanceOf(recipientWallet.address);
  ...
  it("Should not allow non-owner to mint NFTs", async function () {
    try {
      await nftContract.connect(recipientWallet).mint(recipientWallet.address, 4);
      expect.fail("Expected mint to revert, but it didn't");
    } catch (error) {
      expect(error.message).to.include("Only the owner can mint");
    }
  });
