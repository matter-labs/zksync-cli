...
program
  ...
  .action(async (folderName: string, options: CreateOptions) => {
    try {
      options = {
        ...options,
        folderName,
      ...
      runCommand(`git clone ${template.git} ${options.folderName}`);
      runCommand(`cd ${options.folderName} && rm -rf -r .git`); // removes .git folder so new repo can be initialized

      Logger.info("\nInstalling dependencies with yarn...");
      runCommand(`cd ${options.folderName} && yarn`);
